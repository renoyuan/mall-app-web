				...
				<scroll-view class="list-scroll-content" scroll-y @scrolltolower="loadData('add')">
					...
					<view v-for="(item,index) in orderList" :key="index" class="order-item">
						<view class="i-top b-b">
							<text class="time"
								@click="showOrderDetail(item.id)">{{ formatDateTime(item.createTime )}}</text>
						...
						<view class="goods-box-single" v-for="(orderItem, itemIndex) in item.orderItemList"
							...
							<view class="right">
								...
								<text class="attr-box">{{ formatProductAttr(orderItem.productAttr )}} x
...
<script>
	...
	export default {
		...
		onLoad(options) {
			 ...
			 * 替换onLoad下代码即可
			 */
			this.tabCurrentIndex = +options.state||0; // 转为 整行
		...
		methods: {
			...
			loadData(type = 'refresh') {
				...
				console.info("this.orderList ",this.orderList )
				});
			},
			//swiper 切换
			changeTab(e) {
